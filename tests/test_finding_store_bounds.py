...
def test_same_checkpoint_hot_submit_does_not_rescan_unrelated_history(
    ...
    assert second["delivery"]["source_scan_mode"] == "incremental-current-view"
    ...
    assert inbox["source_scan_mode"] == "incremental-current-view"
...
def test_tampered_current_view_falls_back_to_full_history_scan(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    state = _configure_state(tmp_path, monkeypatch)
    worktree = tmp_path / "worktree"
    ...
    cached["findings"][0]["summary"] = "tampered cache"
    target.write_text(
    ...
    assert repaired["source_complete"] is True
    assert repaired["projection_complete"] is True
