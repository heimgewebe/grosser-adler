...
Bei relationalen Aussagen muss das Finding zusätzlich `checkpoint_components` für alle zustandsrelevanten Seiten binden, etwa `local_head` und `upstream_head` oder `pr_head` und `base_head`. Der kanonische `checkpoint_set_sha256` bindet die gesamte Komponentenmenge. Ändert sich auch nur eine abhängige Komponente, gilt `all_components_must_match_or_recheck`: Das Finding darf nicht als weiterhin frisch behandelt werden, sondern benötigt einen Recheck. Einfache, nichtrelationale Findings dürfen aus Kompatibilitätsgründen beim bisherigen einzelnen `checkpoint` bleiben.
